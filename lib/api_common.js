 ...
 */
var API = function (appid, appsecret, getToken, saveToken,routerUrlPrefix) {
  this.appid = appid;
  ...
  this.accessTokenPrefix = routerUrlPrefix || this.prefix;
...
API.prototype.getAccessToken = function (callback) {
  var that = this;
  var url = this.accessTokenPrefix + 'token?grant_type=client_credential&appid=' + this.appid + '&secret=' + this.appsecret;
  this.request(url, {dataType: 'json'}, wrapper(function (err, data) {
    if (err) {
      return callback(err);
    }
    // 过期时间，因网络延迟等，将实际过期时间提前10秒，以防止临界点
    // todo: as a worker, should change the logic that get expireTime from message router
